...
TEST(tagged_ptr, set_tag) {
    auto obj = make_tagged_ptr<std::string, tags<tag<bool, 1>>>("string");
    obj.set_tag<0>(true);
    EXPECT_TRUE(obj.get_tag<0>());
}

TEST(tagged_ptr, get_tag) {
    tagged_ptr<std::string, tags<tag<bool, 1>>> obj;
    obj.set_tag<0>(true);
    EXPECT_TRUE(obj.get_tag<0>());
}
...
TEST(tagged_ptr, bool_operator_return) {
    auto obj = make_tagged_ptr<std::string, tags<tag<bool, 1>, tag<bool, 1>>>();
    EXPECT_TRUE(obj);
    obj.reset();
    EXPECT_FALSE(obj);

    tagged_ptr<std::string, tags<tag<bool, 1>>> false_obj;
    EXPECT_FALSE(false_obj);
}

TEST(tagged_ptr, reset_only_ptr) {
    auto obj = make_tagged_ptr<std::string, tags<tag<bool, 1>, tag<bool, 1>>>("str1");
    EXPECT_EQ(*obj, "str1");
    EXPECT_TRUE(obj);
    obj.set_tag<0>(true);
    EXPECT_TRUE(obj.get_tag<0>());
...
TEST(tagged_ptr, reset) {
    auto obj = make_tagged_ptr<std::string, tags<tag<bool, 1>>>("str");
    EXPECT_TRUE(obj);
    EXPECT_EQ(*obj, "str");
    obj.set_tag<0>(true);
    EXPECT_TRUE(obj.get_tag<0>());
...
TEST(tagged_ptr, set_ptr_value) {
    auto str = make_tagged_ptr<std::string, tags<tag<bool, 1>>>("string");
    *str = "test";
    EXPECT_EQ(*str, "test");
}
