...
public:
    ...
    static constexpr size_t get_flag_size() noexcept {
        using flag_type = typename std::tuple_element<Index, typename Flags::type>::type;
    ...
    static_assert(FlagPtrType::get_flags_size() <= FlagPtrType::BitfieldSize, "flags are too big");
    ...
    void reset_only_ptr(PtrType* ptr) noexcept {
        ...
        const auto flags = (uintptr_t)m_ptr & get_value_mask(flags_size);
        delete_ptr();
...
private:
    ...
    static inline constexpr auto get_value_with_offset_mask(std::size_t size, std::size_t offset) noexcept {
        return get_value_mask(size) << offset;
    }
};

template <typename PtrType, typename FlagsType, bool AutoDestruct=true, typename...Args>
static inline auto make_flag_ptr(Args&&...args) {
   return flag_ptr<PtrType, FlagsType, AutoDestruct>(new PtrType(std::forward<Args>(args)...));
}
